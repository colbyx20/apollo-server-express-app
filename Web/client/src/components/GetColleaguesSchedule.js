...
query GetColleagueSchedule($id: ID) {
  getColleagueSchedule(ID: $id) {
    _id
    time
    room
    coordinatorInfo {
...
export const GetColleaguesSchedule = (props) => {
    ...
    const { getColleagueSchedule } = data;
    ...
    return (
        <TableContainer component={Paper} sx={{ bgcolor: '#231F20', width: '100%' }}>
            <Table sx={{ minWidth: 500 }} size="small" aria-label="a dense table">
                ...
                <TableBody>
                    {getColleagueSchedule.map((coordinator, index) => {
                        return (<TableRow
                            key={coordinator._id}
